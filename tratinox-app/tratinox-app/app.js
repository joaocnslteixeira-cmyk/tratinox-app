...
if (dbRef) {
    ...
    dbRef.on('value', (snapshot) => {
        ...
        if (!data) {
            ...
            localDB = fixPecasLegadas(localDB); // Corrige dados antigos corrompidos
...
// --- MIGRAÇÃO: corrige peças antigas com name/ref corrompidos pelo bug do split('-') ---
function fixPecasLegadas(db) {
    ...
    if (changed) {
        console.log('fixPecasLegadas: peças corrigidas com sucesso.');
        saveLocalBackup(db);
        if (dbRef) dbRef.set(db).catch(() => {});
    }
    return db;
}
...
window.guardarEdicao = function() {
    ...
    if(s) {
        ...
        s.qty = document.getElementById('ed-quantidade').value !== '' ? parseInt(document.getElementById('ed-quantidade').value) : s.qty;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('new-service-form').addEventListener('submit', (e) => {
        ...
        if(!pid) {
            ...
            // Só divide por '-' se houver hífen E ambas as partes forem não-vazias
            const dashIdx = descP.indexOf('-');
            const parsedRef = (dashIdx > 0) ? descP.substring(0, dashIdx).trim() : '';
            const parsedName = (dashIdx > 0 && descP.substring(dashIdx + 1).trim()) 
                ? descP.substring(dashIdx + 1).trim() 
                : descP;
            db.pecas.push({
                id: pid,
                clienteId: cid,
